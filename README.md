...
[`backend/researchlens/models.py`](backend/researchlens/models.py) file. The database schema generated by Django can be found in 
[`backend/researchlens/migrations`](backend/researchlens/migrations) and the final SQL schema that will be generated by Django looks similar to this:
...
We use Django's object relational mapper (ORM) to interact with the database. The ORM allows us to define the database
schema using Python classes and provides a convenient way to query the database in a Pythonic way. Some examples can
be found in the [`backend/researchlens/views.py`](backend/researchlens/views.py) file. For example, all papers sorted 
by published date can be retrieved with the following code:
...
The search functionality is also implemented in [`backend/researchlens/views.py`](backend/researchlens/object_relational_mapper.py).
It uses the Django's functionality to search and filter the papers based on the search criteria. The search criteria can be passed as query parameters to the API endpoint. The pagination is also handled by Django's built-in pagination functionality, which is defined in the `backend/researchlens/pagination.py` file.


### Finding Related Papers
The functionality to get related papers is also implemented in the [`backend/researchlens/views.py`](backend/researchlens/views.py).
